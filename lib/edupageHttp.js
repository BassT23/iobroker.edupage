...
class EdupageHttp {
  ...
  async _storeSetCookies(res, currentUrl) {
    try {
      const setCookie = res?.headers?.['set-cookie'];
      if (!setCookie || !Array.isArray(setCookie) || setCookie.length === 0) return;

      // BONUS (safe): log cookie NAMES only, never values
      try {
        const names = setCookie
          .map(s => String(s).split('=')[0].trim())
          .filter(Boolean);
        if (names.length) this.log.info(`Set-Cookie received: ${names.join(', ')}`);
      } catch {
        // ignore
      }

      // currentUrl may be relative; build absolute for jar scope
      const abs =
        /^https?:\/\//i.test(currentUrl)
          ? currentUrl
          : (this.baseUrl + (currentUrl.startsWith('/') ? currentUrl : `/${currentUrl}`));

      for (const c of setCookie) {
        try {
          // store exactly as server provides it
          await this.jar.setCookie(c, abs);
        } catch {
          // ignore single-cookie parse errors
        }
      }
    } catch {
      // ignore
  ...
  async _requestFollow(method, url, { data, headers, timeout } = {}) {
    ...
    for (let hop = 0; hop < maxHops; hop++) {
      ...
      // POINT 2: always store Set-Cookie from EVERY hop (including redirects)
      await this._storeSetCookies(res, currentUrl);
  ...
  async cookieNamesFor(url) {
    try {
      const abs = /^https?:\/\//i.test(url) ? url : this.baseUrl + url;
      const cookies = await this.jar.getCookies(abs);
      return cookies.map(c => c.key).sort();
    } catch {
      return [];
    }
  }

  async cookieStringFor(url) {
    try {
      const abs = /^https?:\/\//i.test(url) ? url : this.baseUrl + url;
      return await this.jar.getCookieString(abs);
    } catch {
      return '';
    }
  }
